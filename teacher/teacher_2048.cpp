...
board_t convert2board(std::vector<std::vector<int>> obs) {
    board_t board = 0;
    for (int r = 0; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
            int val = obs[r][c];
            uint8_t powerVal = 0;
            if (val > 0) {
                powerVal = static_cast<uint8_t>(val);
            }
            board |= (board_t(powerVal) << (4 * (4 * r + c)));
        }
    }
    return board;
}
...
PYBIND11_MODULE(teacher_2048, m, py::mod_gil_not_used()) {
    m.doc() = "c++ teacher for 2048 game";

    m.def("print_board", &print_board, "A function that prints the current board");
    m.def("convert2board", &convert2board, "A function that converts a tuple of tuple to board_t");
    m.def("find_best_move", &find_best_move, "A function that finds the best move for a given board");
    m.def("init_tables", &init_tables, "A function that initializes the tables");
}